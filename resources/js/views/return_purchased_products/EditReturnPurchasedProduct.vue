       ...
       <div class="bg-white rounded-md shadow-md border-l-8 border-[#022f45] w-full card p-4 mt-3">
        ...
        <div class="flex justify-end">
          <Button 
            ...
            @click="submitEditReturnedProduct"
            :loading="loading"
            />
        </div>
      </div>
...
export default {
  ...
  setup() {
    ...
    const breadcrumbItems = ref([
        ...
        { label: 'Edit Data Return Pembelian' }, 
    ]);
    const router = useRouter()
    const toast = useToast()
    const returnPurchasedProducts = useReturnPurchasedProductsStore()
    const detail_return_purchased_products = computed(() => returnPurchasedProducts.detailReturnPurchasedProduct)
    ...
    const setSeclectedRefNumber = () => {
      if (detail_return_purchased_products.value.return_details.length > 0) {
        selectedRefNumber.value = detail_return_purchased_products.value.return_details.map(detail => detail.reference_number);
      }
    }
    ...
    const { handleSubmit, defineField, errors } = useForm({
      ...
      initialValues: {
        return_reference_number: detail_return_purchased_products.value.return_reference_number,
        supplier_id: detail_return_purchased_products.value.supplier_id,
        apotek_id: detail_return_purchased_products.value.apotek_id,
        return_date: detail_return_purchased_products.value.return_date,
        return_status: detail_return_purchased_products.value.return_status,
        return_note: detail_return_purchased_products.value.return_note,
        additional_note: detail_return_purchased_products.value.additional_note,
      }
    })
    ...
    /**
     * Submits the edited returned product data.
     * 
     * This function handles the submission of edited returned product data. It performs the following steps:
     * 1. Sets the loading state to true.
     * 2. Checks if any products have been selected for return. If not, it displays an error message and exits.
     * 3. Groups the product IDs from the returned products.
     * 4. Calculates the new stock quantities for each product.
     * 5. Constructs the data object for the return, including supplier and apotek IDs, product IDs, reference numbers, return date, status, total items, amount, and notes.
     * 6. Combines the return data with the new stock quantities.
     * 7. Sends the final data to the `editReturnedProduct` method of the `returnPurchasedProducts` service.
     * 8. Handles any errors that occur during the update process, displaying an error message if necessary.
     * 9. If the update is successful, resets the return purchased products list and navigates to the return purchased product data page.
     * 
     * @param {Object} datas - The data object containing the return details.
     */
    const submitEditReturnedProduct = handleSubmit(async (datas) => { 
      loading.value = true
      ...
      await returnPurchasedProducts.editReturnedProduct(finalData, datas.return_reference_number)

      if (returnPurchasedProducts.errorUpdatedData) {
        loading.value = false
        toast.add({
          severity: 'error',
          summary: 'Updated Returned Product Failed',
